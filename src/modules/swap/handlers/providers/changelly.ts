...
class Changelly {
  ...
  getSupportedTokens() {
    return axios
      ...
      .then((response: any) => {
        ...
        const data = response.data.result.filter((d: any) => d.fixRateEnabled);
  ...
  isValidToAddress({ toT, address }: ValidToAddress) {
  ...
  getMinMaxAmount({ fromT, toT }: MinMaxAmountParam) {
    return axios
      .post(
        `${HOST_URL}`,
        {
  ...
  getQuote({ fromT, toT, fromAmount }: QuoteParams) {
      ...
      .then(async (response: any) => {
        ...
        if (!refundAddress) {
          ...
          if (fromT.contract === Configs.ETH) {
            ...
            txObj.value = toHex(
              toWei(response.data.result.amountExpectedFrom, 'ether')
            );
          } else {
            let amountBN: BigNumber | BN | string = new BigNumber(
  ...
  async executeTrade(tradeObj: any, confirmInfo: any) {
