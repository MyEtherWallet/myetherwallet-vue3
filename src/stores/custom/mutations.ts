...
const SET_CUSTOM_TOKEN = function (state: State, { token, rootGetters }) {
...
const DELETE_CUSTOM_TOKEN = function (state: State, { token, rootGetters }) {
  const network = rootGetters["global/network"];
...
const DELETE_ALL_TOKENS = function (
  state: State,
  { rootGetters }: Getter<State, any>
) {
  const network = rootGetters["global/network"];
  let customTokensByNetwork = state.tokens[network.type.name];
  customTokensByNetwork = [];
...
const SET_ADDRESS_BOOK = function (state: State, val) {
  state.addressBook = val;
};

const ADD_CUSTOM_PATH = function (state: State, path) {
  state.paths.push(path);
};

const DELETE_CUSTOM_PATH = function (state: State, paths) {
