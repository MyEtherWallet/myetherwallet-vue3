...
const getEconomy = (gasPrice: any) => {
  return BigNumber(gasPrice).toFixed(0);
};
const getRegular = (gasPrice: any) => {
  if (gasPrice > LIMITER) {
...
const fastToEconomy = (gasPrice: any) => {
...
const regularToEconomy = (gasPrice: any) => {
...
interface GasPriceTypes {
  ECONOMY: 'economy';
  REGULAR: 'regular';
  FAST: 'fast';
}
const gasPriceTypes: GasPriceTypes = {
  ECONOMY: 'economy',
  REGULAR: 'regular',
  FAST: 'fast'
};
const estimatedTime = (type: GasTypes): string => {
  switch (type) {
...
const getGasBasedOnType = (gasPrice: any, gasPriceType: GasTypes) => {
  switch (gasPriceType) {
    case gasPriceTypes.ECONOMY:
...
const getBaseFeeBasedOnType = (baseFeeBN: BN, gasPriceType: GasTypes) => {
  switch (gasPriceType) {
    case gasPriceTypes.ECONOMY:
...
export type GasTypes = 'economy' | 'regular' | 'fast';
//type GasPrice = number | bigint;
export {
