...
const formatIntegerToString = (_value: BigNumber | string | number) => {
 ...
 */
function formatFloatingPointValue(_value: BigNumber | string | number): {
  value: string;
  tooltipText?: string;
} {
  const value = new BigNumber(_value);
 ...
 */
const formatBalanceEthValue = (_value: BigNumber | string | number) => {
  const value = new BigNumber(_value);
 ...
 * @returns {object} FormattedNumber with value as formatted string, unit and tooltipText
 */
const formatGasValue = (_value: BigNumber | string | number) => {
  const value = new BigNumber(_value);
 ...
 * @returns {object} FormattedNumber with value as formatted string, unit and tooltipText
 */
const formatPercentageValue = (_value: string | number) => {
  /* Strip '%' if necessary */
...
const formatFiatValue = (
  _value: BigNumber | number | string,
  ...
  if (value.isGreaterThanOrEqualTo(OneMillion)) {
    return {
      value: localizeCurrency({
        number: formatIntegerValue(value),
        ...options
      })
    };
  }
...
const getRoundNumber = (
  ...
  const dps = value.decimalPlaces() || 0;
  return {
...
const toBNSafe = (number: number | string | BigNumber): BN => {
