...
const stripQuery = (queryObj: any): Query => {
  const newObj: any = {};
...
const createWalletProps = (route: RouteLocation) => {
...
const accessWalletProps = (route: RouteLocation) => {
  if (Object.keys(route.query).length > 0) {
    const { type } = stripQuery(route.query);
    const walletType = type ? type : '';
    const overlay =
...
const accessRouteGuard: NavigationGuard = (to, from, next) => {
  if (to.params.overlay === undefined) {
    next();
  } else {
...
const swapProps = (route: RouteLocation) => {
  if (Object.keys(route.query).length > 0) {
...
const swapRouterGuard: NavigationGuard = (to, from, next) => {
...
const stakedRouterGuard: NavigationGuard = (to, from, next) => {
  if (
    // STAKED => STAKED 1
