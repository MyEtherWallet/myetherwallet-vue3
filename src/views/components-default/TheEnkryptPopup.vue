  ...
  >
    <div class="the-enkrypt-popup popup-container py-10 px-6 px-sm-15">
      ...
      <v-carousel
        ...
        :show-arrows="false"
      >
        <v-carousel-item v-for="(nft, i) in state.nfts" :key="i">
          <img :src="state.nfts[i].src" alt="nft-image" class="main-image" />
        </v-carousel-item>
      </v-carousel>
...
const { trackEnkryptInstall } = useAnalytics();
const { browserLink, browser, browserLogoComputed, openHelpCenter, text } =
  useEnkryptMarketing();
defineProps({
  show: {
    type: Boolean,
...
const close = () => {
  if (popupStore.enkryptLandingPopup) {
    popupStore.neverShowEnkryptLandingPage();
  } else {
    popupStore.neverShowEnkryptWalletPage();
  }
};
</script>
