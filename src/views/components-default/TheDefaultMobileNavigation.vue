      ...
      <v-list color="white" class="px-2">
        ...
        <v-list-item>
          <!-- <v-list-item-content>
            ...
            <a
              v-for="(product, keyProduct) in state.mewProducts"
              :key="keyProduct"
              :href="product.link"
              target="_blank"
...
import { useBuyMore } from '@/core/Common/buyMore';
import { ROUTES_HOME, ROUTES_WALLET } from '@/core/configs/configRoutes';
import { computed, reactive } from 'vue';
import { RouteLocationRaw, useRouter } from 'vue-router';
const { openMoonpay } = useBuyMore();
...
const pushRoute = (to: RouteLocationRaw) => {
  router.push(to).catch(() => true);
  state.isOpen = false;
};
</script>
